...
const gridGenerator = () => {
    ...
    for(let i = 0;i<3;i++) {
        for (let j = 0;j<3;j++){
            ...
            div.innerHTML=`<img src="photos/00${imagesArr[count]}.jpg" class="image ${imagesArr[count]==9 ? "target": ""}" data-index="${imagesArr[count]}"/>`;
            count +=1;
...
const selectImage=(e)=>{
    ...
    if(checkAdjacent(row1,row2,col1,col2)){
        ...
        let targetArrIndex = imagesArr.indexOf(targetArrIndex);
        [imagesArr[currentArrIndex],imagesArr[targetArrIndex],imagesArr[currentArrIndex],];
        if(imagesArr.join("")=="123456789"){
            setTimeout(()=>{
                coverScreen.classList.remove("hide");
                container.classList.add("hide");
                result.innerText=`Total Moves:${movesCount}`;
                startButton.innerText = "RestartGame";},1000);
...
window.onload = () => {
    coverScerrn.classList.remove("hide");
    container.classList.add("hide");
};
